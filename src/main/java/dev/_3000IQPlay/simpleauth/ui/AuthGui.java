package dev._3000IQPlay.simpleauth.ui;

import dev._3000IQPlay.simpleauth.SimpleAuth;
import dev._3000IQPlay.simpleauth.SimpleAuthSpy;
import dev._3000IQPlay.simpleauth.ui.font.CustomFont;
import dev._3000IQPlay.simpleauth.ui.shaders.*;
import dev._3000IQPlay.simpleauth.ui.widgets.*;
import dev._3000IQPlay.simpleauth.util.RoundedShader;
import dev._3000IQPlay.simpleauth.protect.keyauth.KeyAuthApp;
...
public class AuthGui extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		cFont.drawCenteredStringWithShadow("SimpleAuth Auth", width / 2, height / 3.5f + 2, new Color(255, 255, 255, 255).getRGB());

        keyField.drawTextBox();

        cFont.drawCenteredStringWithShadow("DM ClientOwner for any type of Help", width / 2, 10, new Color(255, 255, 255, 255).getRGB());
        ...
        switch (button.id) {
            case 0:
                if (KeyAuthApp.keyAuth.license(keyField.getText())) {
                    SimpleAuth.isOpenAuthGui = false;
	...
	private void saveKey(String key) {
        try {
            FileWriter writer = new FileWriter("SimpleAuth/key.txt");
            writer.write(key);
            writer.close();
        } catch (IOException e) {
            e.printStackTrace();
	...
	private String loadKey() {
        try {
            FileReader reader = new FileReader("SimpleAuth/key.txt");
